...
void VerticalBlurShader::Initialize(D3DGraphics& gfx)
{
	InitializeShaders(gfx, L"Shaders/VerticalBlurVS.cso", L"Shaders/VerticalBlurPS.cso");
}

void VerticalBlurShader::Bind(D3DGraphics& gfx, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float screenHeight)
{
	// 월드, 뷰, 투영 행렬 설정해줌.
	SetShaderParameters(gfx, world, view, projection, texture, screenHeight);

	// 파이프라인에 자원들을 바인딩.
	BindShaders(gfx, indexCount);
}

void VerticalBlurShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	INFOMAN(gfx);
...
void VerticalBlurShader::SetShaderParameters(D3DGraphics& gfx, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float screenHeight)
{
...
void VerticalBlurShader::BindShaders(D3DGraphics& gfx, int indexCount)
{
	// 정점 입력 레이아웃을 바인딩 해줌.
