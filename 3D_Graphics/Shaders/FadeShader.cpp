...
void FadeShader::Initialize(D3DGraphics& gfx)
{
	InitializeShaders(gfx, L"Shaders/FadeVS.cso", L"Shaders/FadePS.cso");
}

void FadeShader::Bind(D3DGraphics& gfx, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float fadeAmount)
{
	// 월드, 뷰, 투영 행렬 설정해줌.
	SetShaderParameters(gfx, world, view, projection, texture, fadeAmount);

	// 파이프라인에 자원들을 바인딩.
	BindShaders(gfx, indexCount);
}

void FadeShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	INFOMAN(gfx);
	...
	fadeBufferDesc.ByteWidth = sizeof(MatrixBufferType);
	...
	// 상수 버퍼 생성.
	gfx.GetDevice()->CreateBuffer(&fadeBufferDesc, NULL, &m_pFadeBuffer);
...
void FadeShader::SetShaderParameters(D3DGraphics& gfx, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float fadeAmount)
{
	...
	GFX_THROW_INFO(gfx.GetContext()->Map(m_pFadeBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
	dataPtr2 = (FadeBufferType*)mappedResource.pData;
	dataPtr2->fadeAmount = fadeAmount;
	gfx.GetContext()->Unmap(m_pFadeBuffer.Get(), 0);

	gfx.GetContext()->PSSetConstantBuffers(bufferNumber, 1, m_pFadeBuffer.GetAddressOf());

	// 픽셀 셰이더에서 사용할 텍스쳐 자원을 설정.
	gfx.GetContext()->PSSetShaderResources(0, 1, &texture);
}

void FadeShader::BindShaders(D3DGraphics& gfx, int indexCount)
{
	// 정점 입력 레이아웃을 바인딩 해줌.
