...
void TranslateShader::Initialize(D3DGraphics& gfx)
{
	InitializeShaders(gfx, L"Shaders/TranslateVS.cso", L"Shaders/TranslatePS.cso");
}

void TranslateShader::Bind(D3DGraphics& gfx, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float translationX, float translationY)
{
	SetShaderParameters(gfx, world, view, projection, texture, translationX, translationY);
	
	BindShaders(gfx, indexCount);
}

void TranslateShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	INFOMAN(gfx);
	...
	// 상수 버퍼 생성.
	gfx.GetDevice()->CreateBuffer(&translateBufferDesc, NULL, &m_pTranslationBuffer);
...
void TranslateShader::SetShaderParameters(D3DGraphics& gfx, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float translationX, float translationY)
{
	...
	// 상수 버퍼에 값을 쓰기 위해 lock.
	GFX_THROW_INFO(gfx.GetContext()->Map(m_pTranslationBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
	...
	// 값을 다 썼으므로, 잠궈준 것을 다시 풀어줌. unlock.
	gfx.GetContext()->Unmap(m_pTranslationBuffer.Get(), 0);

	// 정점 셰이더에서의 상수 버퍼의 위치를 설정.
	bufferNumber = 0;

	// 정점 셰이더에 있는 상수 버퍼를 갱신된 값으로 설정해줌.
	gfx.GetContext()->PSSetConstantBuffers(bufferNumber, 1, m_pTranslationBuffer.GetAddressOf());

	// 픽셀 셰이더에서 사용할 텍스쳐 자원을 설정.
	gfx.GetContext()->PSSetShaderResources(0, 1, &texture);
}

void TranslateShader::BindShaders(D3DGraphics& gfx, int indexCount)
{
	// 정점 입력 레이아웃을 바인딩 해줌.
