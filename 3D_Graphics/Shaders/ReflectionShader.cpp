...
void ReflectionShader::Initialize(D3DGraphics& gfx)
{
	InitializeShaders(gfx, L"Shaders/ReflectionVS.cso", L"Shaders/ReflectionPS.cso");
}

void ReflectionShader::Bind(D3DGraphics& gfx, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTexture, XMMATRIX reflection)
{
	SetShaderParameters(gfx, world, view, projection, texture, reflectionTexture, reflection);

	// 파이프라인에 자원들을 바인딩.
	BindShaders(gfx, indexCount);
}

void ReflectionShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	INFOMAN(gfx);
	...
	// 정점 셰이더에서 사용할 행렬들을 담은 상수 버퍼에 대한 서술자를 세팅.(반사 뷰 행렬)
...
void ReflectionShader::SetShaderParameters(D3DGraphics& gfx, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTexture, XMMATRIX reflection)
{
	...
	gfx.GetContext()->PSSetShaderResources(1, 1, &reflectionTexture);
}

void ReflectionShader::BindShaders(D3DGraphics& gfx, int indexCount)
{
	// 정점 입력 레이아웃을 바인딩 해줌.
