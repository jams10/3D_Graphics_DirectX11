...
void InstanceTextureShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	...
	polygonLayout[2].SemanticName = "TEXCOORD"; // 시멘틱 이름은 범용 시멘틱인 TEXCOORD 지정.
	polygonLayout[2].SemanticIndex = 1; // 위에서 텍스쳐 좌표에 사용하는 TEXCOORD 시멘틱이 0번 슬롯을 사용하므로 여기에서는 1번 슬롯으로 지정함.
	polygonLayout[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	polygonLayout[2].InputSlot = 1;
	polygonLayout[2].AlignedByteOffset = 0; // position은 인스턴스 버퍼의 첫 데이터이며, 정점 버퍼와 인스턴스 데이터를 정렬하지 않을 것이기 때문에 정렬 옵션을 0으로 설정.
	polygonLayout[2].InputSlotClass = D3D11_INPUT_PER_INSTANCE_DATA; // 이 데이터가 인스턴스 데이터임을 표시.
	polygonLayout[2].InstanceDataStepRate = 1; // 인스턴스 데이터 당 하나의 인스턴스를 그리도록 함.
