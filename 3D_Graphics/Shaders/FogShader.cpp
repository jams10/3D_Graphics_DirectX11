...
void FogShader::Initialize(D3DGraphics& gfx)
{
	InitializeShaders(gfx, L"Shaders/FogVS.cso", L"Shaders/FogPS.cso");
}

void FogShader::Bind(D3DGraphics& gfx, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float fogStart, float fogEnd)
{
	SetShaderParameters(gfx, world, view, projection, texture, fogStart, fogEnd);
	BindShaders(gfx, indexCount);
}

void FogShader::InitializeShaders(D3DGraphics& gfx, const std::wstring& vsFileName, const std::wstring& psFileName)
{
	INFOMAN(gfx);
	...
	// 정점 셰이더에서 사용할 안개 정보를 담은 상수 버퍼에 대한 서술자를 세팅.
	...
	// 상수 버퍼 생성.
	GFX_THROW_INFO(gfx.GetDevice()->CreateBuffer(&fogBufferDesc, NULL, &m_pFogBuffer));
...
void FogShader::SetShaderParameters(D3DGraphics& gfx, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* texture, float fogStart, float fogEnd)
{
	...
	// 상수 버퍼에 값을 쓰기 위해 lock.
	GFX_THROW_INFO(gfx.GetContext()->Map(m_pFogBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));

	// lock을 통해 얻어온 매핑된 리소스에서 상수 버퍼의 데이터에 대한 포인터를 얻어옴.
	dataPtr2 = (FogBufferType*)mappedResource.pData;

	// 매핑된 메모리에 fog 값들을 업데이트 해줌.
	dataPtr2->fogStart = fogStart;
	dataPtr2->fogEnd = fogEnd;

	// 값을 다 썼으므로, 잠궈준 것을 다시 풀어줌. unlock.
	gfx.GetContext()->Unmap(m_pFogBuffer.Get(), 0);

	bufferNumber = 1; // 행렬 상수 버퍼 다음으로 들어가야 하므로 버퍼 숫자를 하나 늘려줌.
	gfx.GetContext()->VSSetConstantBuffers(bufferNumber, 1, m_pFogBuffer.GetAddressOf());
}

void FogShader::BindShaders(D3DGraphics& gfx, int indexCount)
{
	// 정점 입력 레이아웃을 바인딩 해줌.
